...
test('slow score saves leave other cells editable, preserve focus, and serialize revisions',async()=>{
 ...
 w.eval(fs.readFileSync('js/trip-o-meter.js','utf8'));const period=w.document.querySelector('[data-period-select]');period.value='1';period.dispatchEvent(new w.Event('change'));await tick();
 ...
 pending.shift()();await tick();assert.match(w.document.querySelector('[data-save-notice]').textContent,/All scores saved/);assert.match(w.document.querySelector('.trip-score-breakdown summary').textContent,/50%/);assert.equal(w.document.activeElement,cells[1]);dom.window.close();
});

(() => {
  ...
  async function load(){
    ...
    if(!window.NestAuth?.identity?.signedIn){status.replaceChildren('Sign in to view your division’s report data. ');const sign=document.createElement('button');sign.type='button';sign.textContent='NEST sign in';sign.addEventListener('click',()=>window.NestAuth?.open());status.append(sign);return;}

(() => {
  ...
  dialog.innerHTML = `<div class="nest-auth-card"><button type="button" class="nest-auth-close" aria-label="Close">×</button>
    <h2>NEST Login</h2><p data-message role="status" aria-live="polite">Sign in to use your NEST tools.</p>
  ...
  function mode(value) {
    ...
    message.textContent = value === 'login' ? 'Sign in to use your NEST tools.' : value === 'register-request' ? 'Enter your district email to create an account.' : value === 'register-verify' ? 'Enter the code sent to your district email.' : 'Choose your NEST username and password.';
  }
  function open(selected = 'login') { mode(selected); if (!dialog.open) dialog.showModal(); }
  ...
  const ready = fetch('/api/auth?action=me', { credentials: 'same-origin', cache: 'no-store' }).then(response => response.json()).then(data => { identity = data.signedIn ? data : null; refreshLinks(); changed(); return identity; }).catch(() => { refreshLinks(); changed(); return null; });
  window.NestAuth = { ready, open, logout, get identity() { return identity; } };
})();
